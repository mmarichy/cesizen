...
export async function GET() {
  try {
    ...
    console.error("Erreur GET /api/account/activity-favorites:", error);
    return NextResponse.json(
...
export async function POST(request: Request) {
  try {
    ...
    console.error("Erreur POST /api/account/activity-favorites:", error);
    return NextResponse.json(
...
export async function DELETE(request: Request) {
  try {
    ...
    const activityId =
      new URL(request.url).searchParams.get("activityId") ?? "";
    if (!activityId.trim()) {
      return NextResponse.json(
        { message: "activityId requis" },
        { status: 400 },
    ...
    console.error("Erreur DELETE /api/account/activity-favorites:", error);
    return NextResponse.json(
