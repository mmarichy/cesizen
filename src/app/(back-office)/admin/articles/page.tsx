...
import { Clock3, Pencil, Plus, Power } from "lucide-react";
import { AdminSearchInput } from "@/components/back-office/admin-search-input";
import { DeleteArticleButton } from "@/components/back-office/articles/delete-article-button";
import { useAdminArticles } from "@/components/back-office/articles/use-admin-articles";
...
async function toggleArticleArchivedAction(formData: FormData) {
  ...
  if (!response.ok) {
    throw new Error(getMessage(data, "Impossible de mettre à jour le statut de l'article."));
  }
}
...
async function deleteArticleAction(formData: FormData) {
  ...
  const data = (await response.json()) as { message?: string };
  if (!response.ok) {
    throw new Error(getMessage(data, "Impossible de supprimer l'article."));
  }
}
...
export default function ArticlesAdminPage() {
  ...
  const {
    ...
    goToPreviousPage,
    goToNextPage,
  } = useAdminArticles(debouncedQuery);
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <section className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-3">
          <h2 className="text-2xl font-bold text-gray-900 sm:text-3xl">Liste des articles</h2>
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="mb-6 flex flex-col gap-4 border-b border-gray-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <PaginationControls
            summary={`Page ${page} sur ${totalPages} - ${totalArticles} article${totalArticles > 1 ? "s" : ""}`}
            currentPage={page}
          ...
          <>
            <div className="mt-6 space-y-4 md:hidden">
              {articles.map((article) => {
                ...
                return (
                  <article key={article.id} className="rounded-2xl border border-gray-200 p-4 shadow-xs">
                    <div className="flex items-start justify-between gap-3">
                      <p className="line-clamp-2 text-sm font-semibold text-gray-900">{article.title}</p>
                      <span
                        className={[
                          "inline-flex shrink-0 rounded-full px-3 py-1 text-xs font-semibold",
                          statusBadgeClassName(article.status),
                        ].join(" ")}
                      >
                    ...
                    <div className="mt-3 space-y-3 text-sm text-gray-700">
                      <p>
                        <span className="font-semibold text-gray-900">Auteur: </span>
                        {article.author}
                      </p>
                      <p>
                    ...
                    <div className="mt-4 flex min-w-0 items-center gap-2">
                      ...
                      <form
                        action={async (formData) => {
                          try {
                            await toggleArticleArchivedAction(formData);
                            refresh();
                          } catch (error) {
                            toast.error(
                      ...
                      >
                        ...
                        <button
                          ...
                          className={[
                            ...
                            toggleButtonClassName(!isArchived),
                          ].join(" ")}
                        >
              ...
              <table className="w-full table-auto border-separate border-spacing-0">
                <thead>
                  <tr className="text-sm font-semibold text-gray-700">
                    <th className="w-[25%] max-w-56 px-3 py-3 text-left">Titre</th>
                    <th className="px-3 py-3 text-left">Auteur</th>
                  ...
                  {articles.map((article) => {
                    ...
                    return (
                      <tr key={article.id} className="border-t border-gray-100">
                        ...
                        <td className="px-3 py-5 text-sm text-gray-700">{article.author}</td>
                        <td className="px-3 py-5 text-sm text-gray-700">{article.tag}</td>
